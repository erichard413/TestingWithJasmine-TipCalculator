...
//Click X td element to remove TR from #ServerTable
const serverTable = document.querySelector('#serverTable tbody');
serverTable.addEventListener('click', removeServer);

function removeServer(e){
  if (e.target.innerText === 'X'){
  removeThis = (e.target).parentNode
  serverTable.removeChild(removeThis);
  deleteThis = ((e.target).parentNode.id);
  delete allServers[deleteThis]
  updateServerTable();
